...
def main(args, config):
    ...
    print(f"Validation dataset size: {len(datasets[1])}")

    print(f"train_loader {len(train_loader)}")
    # i=0
    # for i, batch in enumerate(train_loader):
    #     print(f"Batch {i}: {batch}")
    #     if i >= 1:  # 첫 1~2개의 배치만 확인
    #         break
    ...
    # 첫 번째 배치의 구조와 데이터 타입 출력
    # print(f"Batch structure: {type(first_batch)}")
    ...
    # train_step_per_epoch 계산
    train_step_per_epoch = len(train_loader)

    if train_step_per_epoch == 0:
    	raise ValueError("Train loader length is 0. Check your dataset and loader settings.")
    ...
    # 스케줄러 설정
    arg_sche = config['schedular']
    arg_sche["t_initial"] = max_epoch * train_step_per_epoch
    arg_sche["warmup_t"] = warmup_steps * train_step_per_epoch  # warmup_t도 에폭 단위로 설정
