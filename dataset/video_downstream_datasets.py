...
class video_caption_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if not video_id:
        # # Handle missing video_id appropriately
        # # For example, skip this entry, log a warning, or use a placeholder
        #     print(f"Warning: Missing 'video_id' for index {index}. Skipping.")
...
class video_caption_dataset(Dataset):
    #def __init__(self, ann_file, transform, video_root, num_frames=16, split='train', max_words=30, read_local_data=True):
    def __init__(self, json_path, transform, video_root='', num_frames=16, split='train', max_words=30, read_local_data=True):    
        # self.ann = load_jsonl(ann_file)
        # self.transform = transform
        # self.max_words = max_words
        # self.video_root = video_root
        ...
        self.videos = data["videos"]  # 비디오 정보
        self.sentences = data["sentences"]  # 캡션 정보
        self.transform = transform
        self.video_root = video_root
        self.num_frames = num_frames
        self.max_words = max_words
        ...
        # 비디오 ID를 키로 캡션을 매핑하는 딕셔너리 생성
        self.caption_dict = {}
        for caption_info in self.sentences:
            video_id = caption_info["video_id"]
            if video_id not in self.caption_dict:
                self.caption_dict[video_id] = []
            self.caption_dict[video_id].append(caption_info["caption"])
        
        # 현재 split에 해당하는 비디오만 필터링
        self.videos = [video for video in self.videos if video['split'] == self.split]
    ...
    def __getitem__(self, index):
        ...
        # 비디오에 해당하는 캡션 리스트를 가져옴
        captions = self.caption_dict.get(video_id, [])
        ...
        if self.split == 'train':
            ...
            while True:
                ...
                try:
                    ...
                    continue
                break
            #caption = pre_caption(captions, 80)
            processed_captions = [pre_caption(caption.lower(), 80) for caption in captions]

            if self.transform:
                video_array = self.transform(video_array) # (T, C, H, W) -> (C, T, H, W)

            return video_array, processed_captions
